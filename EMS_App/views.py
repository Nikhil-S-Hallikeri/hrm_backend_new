    ...
    def get(self, request, emp_info_id):
        try:
            ...
            employee_information = EmployeeInformation.objects.prefetch_related(
                ...
                ).filter(lookup_query).first()
            
            if not employee_information:
                return Response("employee id not exist", status=status.HTTP_400_BAD_REQUEST)
            ...
            if employee_information.ProfileVerification != "success":
                ...
                if not hasattr(employee_information, 'employeedatamodel'):
                     return Response("profile verification has to be done.", status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                emp_data = EmployeeDataModel.objects.select_related('Position__Department', 'Reporting_To').filter(EmployeeId=employee_information.employee_Id).first()

            register_obj = RegistrationModel.objects.filter(EmployeeId=employee_information.employee_Id).first()
            ...
            try:
                ...
                direct_reports = EmployeeDataModel.objects.filter(Reporting_To=emp_data.pk)
                has_direct_reports = direct_reports.exists()
                sub_rep_to = EmployeeDataModel.objects.filter(Reporting_To__Reporting_To=emp_data.pk).exists()
                ...
                elif has_direct_reports and (emp_data.Designation == "Employee" or emp_data.Designation == "Recruiter"):
                    Status = "team_leader"
                ...
                employee_profile["Department"]=emp_data.Position.Department.Dep_Name if emp_data.Position and emp_data.Position.Department else None
...
class LoginEmployeeProfileView(APIView):
    def get(self, request, loginuser):
        try:
            ...
            Employee = EmployeeDataModel.objects.select_related(
                'employeeProfile', 'Position__Department', 'Reporting_To'
            ).filter(EmployeeId=loginuser).first()
            ...
            employee_profile["Position"] = Employee.Position.Name if Employee.Position else None
            employee_profile["Department"] = Employee.Position.Department.Dep_Name if Employee.Position and Employee.Position.Department else None
